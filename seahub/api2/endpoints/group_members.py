...
class GroupMembers(APIView):
    ...
    def get(self, request, group_id, format=None):
        ...
        if is_admin.lower() not in ('true', 'false'):
            error_msg = _(u'"is_admin" argument can only be "true" or "false".')
...
class GroupMember(APIView):
    ...
    def put(self, request, group_id, email):
        """
        Set/unset a specific group member admin.
        """
    ...
    def post(self, request, group_id):
        ...
        for email in emails_list:
            ...
            if org_id and not \
                ...
                result['failed'].append({
                    'email': email,
                    'error_msg': 'Is not an organizaiont user'
                    })
                continue
